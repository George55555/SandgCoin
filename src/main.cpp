...
unsigned int nTargetSpacing = 4 * 60; // SandGCoin - 4 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 8 * 60 * 60; // 8 hours
unsigned int nStakeMaxAge = -1;
unsigned int nModifierInterval = 10 * 60; // SandGCoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "SandGCoin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SandGCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1434407596;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 11828701;
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x9e2800d581c72822a687cf3f2f7b28043984bbf5d7eb28fe2b536354d9b8a89a"));
